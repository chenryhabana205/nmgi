...
echo "Building ${_docker_repo}:${_nmgi_version}-slim"

#run from scrash
# docker build \
# 	--tag "${_docker_repo}:${_nmgi_version}-slim" \
# 	--no-cache=true .


docker build -f docker/slim/Dockerfile \
	--tag "${_docker_repo}:${_nmgi_version}-slim" .
...
services:
  nmgi:
    image: ${_docker_repo}:${_nmgi_version}-slim
...
echo "Complete ${_docker_repo}:${_nmgi_version}-slim"